...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<WarehouseInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/warehouses` : null),
    () => getWarehouses({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Warehouses',
      path: isTenantUser ? `/warehouses/view/${data?.[0]?.id}` : '/warehouses',
      entity: 'warehouse',
