...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Product Addition.create': {
    roles: ['business-owner', 'shipping-coordinator', 'receiving-clerk'],
    key: 'product_addition',
    tenantPath: [],
    userPath: [],
  },
  'Shipment Dispatch.update': {
    roles: ['business-owner', 'inventory-manager', 'receiving-clerk'],
    key: 'shipment_dispatch',
    tenantPath: [],
    userPath: [],
  },
  'Shipment Received.update': {
    roles: ['business-owner', 'inventory-manager', 'shipping-coordinator'],
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.warehouse.findFirst({
